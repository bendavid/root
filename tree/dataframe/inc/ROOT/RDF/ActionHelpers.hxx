...
public:
   ...
   void Exec(unsigned int slot, const Xs&... xs) {
      ...
      constexpr auto find_idx_true = [](auto const& arr) {
      ...
      constexpr size_t colidx = find_idx_true(isContainer);
      ...
      auto const end0 = std::end(std::get<colidx>(xst));
      
      auto get_size = [](auto const& x)->size_t {
         if constexpr(std::is_convertible<decltype(x), double>::value) {
            return 1;
         }
         else {
            return std::size(x);
      ...
      const std::array<size_t, sizeof...(xs)> sizes = {{ get_size(xs)... }};
      ...
      // increment all iterators in tuple
      auto increment = [](auto&... its) {
         auto increment_one = [](auto &it) {
            if constexpr(std::is_convertible<decltype(it), double>::value) {
               ;
            }
            else {
               ++it;
      ...
      // fill the object
      auto fill = [thisSlotH](auto&... its) {
        
         auto deref_one = [](auto it) {
            if constexpr(std::is_convertible<decltype(it), double>::value) {
               return it;
            }
            else {
         ...
         thisSlotH->Fill(deref_one(its)...);
      };

      for (auto itst = std::make_tuple(make_begin(xs)...); std::get<colidx>(itst) != end0; std::apply(increment, itst)) {
         std::apply(fill, itst);
      }
