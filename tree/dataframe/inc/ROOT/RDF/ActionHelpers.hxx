...
public:
   ...
   const T &MakeBegin(const T &val) {  return val; }

   // return iterator to beginning of container
   template<typename T, typename std::enable_if<!std::is_convertible<T, double>::value, int>::type = 0>
   typename T::const_iterator MakeBegin(const T& val) {
      return std::begin(val);
   }

   // return unchanged value for scalar
   template<typename T, typename std::enable_if<std::is_convertible<T, double>::value, int>::type = 0>
   const T &Increment(const T &val) { return val; }
   ...
   // return unchanged value for scalar
   template<typename T, typename std::enable_if<std::is_convertible<T, double>::value, int>::type = 0>
   const T &Deref(T &val) { return val; }

   // return dereferenced value for iterator
   template<typename T, typename std::enable_if<!std::is_convertible<T, double>::value, int>::type = 0>
   const auto &Deref(const T& val) { return *val; }

   // return 1 for scalars
   template<typename T, typename std::enable_if<std::is_convertible<T, double>::value, int>::type = 0>
   std::size_t GetSize(const T&) { return 1; }
   ...
   void ExecLoop(unsigned int slot, End_t end, Its... its) {
      ...
      auto nop = [](auto&&...) {};
      for (; GetNthElement<ColIdx>(its...) != end; nop(Increment(its)...) ) {
