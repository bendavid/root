...
public:
   ...
   // with arguments
   template <typename... T>
   Longptr_t ExecPluginT(T... params)
   {
      constexpr auto nargs = sizeof...(params);
      if (!CheckForExecPlugin((Int_t)nargs))
         return 0;
      ...
      if (tupleclass != fArgTupleClasses[nargs - 1]) {
         if (gDebug > 1) {
            Warning("ExecPluginT", "Mismatched argument types %s vs %s", tupleclass->GetName(),
                    fArgTupleClasses[nargs - 1]->GetName());
         }
         return 0;
      }
